...
pre-registration (ADR 0018). The submission horizon has moved: ~18 months
remain (target ≈ December 2027). The current thesis is *diagnosis + design*.
The surplus time is enough to upgrade it to *diagnosis + tested prediction +
causal evidence* — but only if the confirmatory-critical reasoning is frozen
now, while the high-capability reasoning seat is still available (days, not
months), and execution is routed to cheaper seats (Codex for mechanical work,
mid-tier models for drafting/interpretation) against oracles the reasoning
seat writes.

Candidate arms considered:

- **Arm 1 — run Direction B for real.** Stage 1 (online vignette, closes
...
- **Arm 2 — out-of-sample test of the transience/durability claim.** Extend
  ...
  into tested prediction (or falsifies it — either outcome is thesis-grade).
...
- **Arm 4 — PBL review-corpus as convergent-validity layer.** Correlate
  ...
  the survey tags). Measurement validation only — never causal evidence
...
**Phase 0 — reasoning-seat window (now, days).** Produce, in priority order,
the artifacts whose errors are irreversible or expensive:
...
1. **ADR 0020 — frozen out-of-sample prediction criteria for Arm 2.** The
   ...
   time-critical reasoning task: once new data is seen, the option to
   pre-specify is gone forever.
2. **Arm 3 design spec.** Template-transfer definition: what must be held
   ...
   the honest limit that 2015 lacks the mobile-panel layer, so the Kanazawa
...
- Codex: data-pull scaffolding for extended JTA/FTAS/mobile windows —
  built and tested, but post-2024-extension outcomes stay unopened until
...
- Arm 4 if capacity: free — reuses committed aggregates and existing
  pipelines; no new data spend.

**Phase 3 — 2027-04 → 2027-09.** Stage 2 analysis (frozen plan), Chapter 6
rewrite as results, Arm 2/3 results chapters, revise-and-resubmit handling.

**Phase 4 — 2027-10 → 2027-12.** Integration pass (successor to ADR 0014),
...
- **TripAdvisor:** fails (b) as proposed — skews to the same
...
- **Korean sources (e.g., Naver blog/café search):** the only genuinely new
...
- **Reasoning seat (now):** Phase 0 artifacts only — frozen rules, designs,
  contingency trees, chapter architecture. Nothing mechanical.
...
- **Mid-tier (Opus-class, later):** interpretation memos as results land,
  chapter drafting against the Phase 0 architecture, journal-response
  drafting. Escalate to the best available seat for any decision that
  amends a frozen rule (each such amendment is a new ADR and demotes the
  affected analysis to exploratory, per ADR 0018's deviation discipline).
...
- Arm 1 is governed entirely by ADR 0018; this roadmap adds no new analysis
  ...
  prereg and the thesis reverts to design-not-evidence framing — which is
...
- Arm 2 proceeds only after ADR 0020 is accepted; if the prediction fails,
  ...
  contribution).
- Arm 3 is killed if the data-availability audit shows the pre-period
  cannot support the fit gates; it is a publication enhancer, not a
...
- The scarce reasoning window is spent exclusively on artifacts that cannot
  be recovered later (pre-specification, designs, contingency trees), not
  on execution that cheaper seats can do against oracles.
...
- **Journal submission after Stage 2 results (late 2027):** wastes the
  review cycle as mock defense; existing Chapters 3–4 are already
  publishable.
