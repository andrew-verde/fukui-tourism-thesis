...
| JTA Arm 3 vintage oracle: 7 annual releases (2011–2017) | Corrected confirmed-value annual files staged for the 推移表 cross-check; dataset provenance only, not an empirical finding | verified | `python scripts/fetch_national_direct.py --source jta_accommodation_2011_confirmed --source jta_accommodation_2012_confirmed --source jta_accommodation_2013_confirmed --source jta_accommodation_2014_confirmed --source jta_accommodation_2015_confirmed --source jta_accommodation_2016_confirmed --source jta_accommodation_2017_confirmed` | `scripts/fetch_national_direct.py` | Seven checksum-pinned JTA 宿泊旅行統計調査 URLs in `config/national_data_sources.yaml` | `output/national_stats/raw/jta_accommodation_2011_confirmed.xls` … `jta_accommodation_2017_confirmed.xlsx`; `output/national_stats/direct_manifest.json` |
| Arm 3 Kanazawa replication: V1a +10.212% p = 0.181818; V2a +10.511% p = 0.212121; V1b p = 0.580645; V1c range [+6.645%, +11.134%] | Ishikawa 2015 template replication at prefecture grain — **V1 fail, V2 fail** (ADR 0031); portability not demonstrated | verified (null result; pre-specified per ADR 0021 §6) | `make arm3-kanazawa` | `scripts/arm3_kanazawa_scm.py` | `output/national_stats/raw/jta_accommodation_timeseries.xlsx` (推移表, ≤ 201912 slice) + 7 pinned JTA annuals | `output/arm3_kanazawa/causal_robustness/metrics.json`, `specification_summary.csv` |
